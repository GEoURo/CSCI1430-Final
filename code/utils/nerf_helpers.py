...
def meshgrid_xy(tensor1: torch.Tensor, tensor2: torch.Tensor) -> (torch.Tensor, torch.Tensor):
    """
    Mimic np.meshgrid(..., indexing="xy") in pytorch. torch.meshgrid only allows "ij" indexing.
...
def get_ray_bundle(height: int, width: int, focal_length: float, tform_cam2world: torch.Tensor):
    ...
    datatype = tform_cam2world.dtype
    ii, jj = meshgrid_xy(torch.arange(width, dtype=datatype, device=device),
                         torch.arange(height, dtype=datatype, device=device))

    # naive ray directions in camera space
    ...
    # what is this doing? Why sum? why expand another dimension
