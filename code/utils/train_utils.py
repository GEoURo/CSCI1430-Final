import torch
from .nerf_helpers import ndc_rays
from .nerf_helpers import sample_pdf_2 as sample_pdf
from .volume_rendering_utils import volume_render_radiance_field
from typing import Optional


def get_mini_batches(inputs: torch.Tensor, chunksize: Optional[int] = 1024 * 8):
    """
    Takes a huge tensor (ray "bundle") and splits it into a list of mini batches.
...
def runModel(model, pts, ray_batch, chunksize):
    ...
    chunk = torch.cat((chunk, input_dirs_flat), dim=-1)

    batches = get_mini_batches(chunk, chunksize=chunksize)
    preds = [model(batch[..., 0:3], batch[..., 3:6]) for batch in batches]
...
def run_one_iter_of_nerf(
    ...
    batches = get_mini_batches(rays, chunksize=getattr(options.nerf, mode).chunksize)
    pred = [predict_and_render_radiance(batch, model_coarse, model_fine, options) for batch in batches]
